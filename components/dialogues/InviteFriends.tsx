...
export default function InviteFriends({ navigation }) {
  ...
  const icon = (props) => (
    <Avatar.Icon {...props} icon="email-plus-outline" size={responsiveFontSize(5)} />
  );
  ...
  const sendInvite = () => {
    ...
    setInvitedFriends(prevState => ({ ...prevState, [selectedFriend]: true }));
        ...
        <View style={styles.section}>
          ...
          <ScrollView style={styles.friendsListContainer}>
            {filteredFriends.map((friend, index) => (
              <View key={index}
                style={[styles.friendItem2, invitedFriends[friend] ? styles.invitedFriendItem : null]}>
                <FriendItem2
                  key={index}
                  friend={friend}
                  ...
                  onIconPress={() => !invitedFriends[friend] && inviteFriend(friend)}
                />
              </View>
            ))}
          </ScrollView>
